...
db.ref("config/promocaoAtual").on("value", async (snap) => {
  if (snap.exists()) promocaoAtual = snap.val();
  await carregarTodosComentariosPublicos();
});
...
btnEntrarAdmin.addEventListener("click", async () => {
  const loginOk = loginAdmin.value.trim().toLowerCase() === LOGIN_ADMIN;
  const senhaOk = senhaAdmin.value.trim() === SENHA_ADMIN;
  if (!loginOk || !senhaOk) {
    adminErro.textContent = "Login ou senha incorretos.";
    return;
  }
...
window.excluirUmaAvaliacao = async function(key, promocaoId, usuarioKey) {
  ...
  try {
    ...
    await db.ref("avaliacoes/" + key).remove();
